...
`modpack use <id>` stores the validated active project in the Nu session as `MODPACKTOOLS_PROJECT`. Child PowerShell bridge processes inherit that variable when the module is imported, so the adapter does not rewrite later commands or inject `--project` arguments.
