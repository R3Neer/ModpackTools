    ...
    $transaction = Invoke-MpProjectTransaction $Project -DryRun:$DryRun -ExpectedChanges $ExpectedChanges -Prepare {
        ...
        if ($Operation -eq 'update') {
            ...
            foreach ($target in $targets) {
                if ($initial.Nodes[$target.Id].Pinned) { Throw-MpError -Message "'$($target.Name)' is pinned" -Hint "modpack content unpin $($target.Id)" -ErrorId 'Compatibility.Pinned' -Category InvalidOperation }
        ...
        if ($Enable -and @($plan.Requested | Where-Object { $plan.Nodes[$_].Item.Kind -ne 'resourcepack' }).Count) {
            Throw-MpError -Message '--enable-at requires a resource pack batch' -Hint 'add mods and shaders separately' -ErrorId 'Option.EnableRequiresResource' -Category InvalidArgument
        }
...
function Invoke-MpAdd {
    ...
    $parsed = ConvertFrom-MpOptions $Arguments -ValueOptions @('category','enable-at') -SwitchOptions @('dry-run','strict','allow-downgrade')
    Assert-PositionalCount $parsed.Positionals -Minimum 1 -Maximum ([int]::MaxValue) -Usage 'modpack content add <selector...> [options]'
    ...
    $position = 0
    if ($parsed.Options.ContainsKey('enable-at') -and (-not [int]::TryParse([string]$parsed.Options['enable-at'], [ref]$position) -or $position -lt 1)) { Throw-MpError -Message 'Enable position must be a positive integer' -Hint '--enable-at <n>' -ErrorId 'Option.InvalidPosition' -Category InvalidArgument }
    ...
    $result = Invoke-MpContentOperation $project -Operation add -Selectors $selectors -Category $category -Enable:$parsed.Options.ContainsKey('enable-at') -Position $position -Strict:$parsed.Options.ContainsKey('strict') -AllowDowngrade:$parsed.Options.ContainsKey('allow-downgrade') -DryRun:$parsed.Options.ContainsKey('dry-run')
...
function Invoke-MpUpdate {
    param([Parameter(ValueFromRemainingArguments)][object[]]$Arguments = @())
    $parsed = ConvertFrom-MpOptions $Arguments -ValueOptions @('type','to') -SwitchOptions @('all','dry-run','strict','allow-downgrade')
    if (($parsed.Options.ContainsKey('all') -and $parsed.Positionals.Count) -or (-not $parsed.Options.ContainsKey('all') -and -not $parsed.Positionals.Count)) { Throw-MpError -Message 'Specify selectors or --all' -Hint 'modpack content update --help' -ErrorId 'Option.ForbiddenCombination' -Category InvalidArgument }
    if ($parsed.Options.ContainsKey('to') -and ($parsed.Options.ContainsKey('all') -or $parsed.Positionals.Count -ne 1)) { Throw-MpError -Message '--to requires exactly one selector' -Hint 'modpack content update <selector> --to <version>' -ErrorId 'Option.VersionTargetConflict' -Category InvalidArgument }
    $project = Resolve-MpCommandProject $parsed.Options
...
function Write-MpContentPlan {
    ...
    if (-not $SkipHealth -and $Plan.Report -and $Plan.Report.PSObject.Properties['Errors'] -and $Plan.Report.PSObject.Properties['Unknown'] -and $Plan.Report.PSObject.Properties['Warnings']) {
        foreach ($item in @(Get-MpHealthDisplayItems $Plan.Report -Compact)) { Write-MpDoctorItem -Status $item.Status -Text $item.Text }
    }
}